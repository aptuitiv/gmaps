...
- `loader().load()` now settles every caller when a load fails. Callers arriving while a load was already running waited on the "load" event, which only fires on success, so a failed load left all but the first of them waiting for something that was never coming. They now share the load that is in flight and are rejected with it. A failed load is also let go of, so a later call starts a fresh attempt rather than being handed the failure again. It used to be left set, so every call after a failure waited for a "load" event that was never going to be dispatched, and nothing could make the loader try again.
