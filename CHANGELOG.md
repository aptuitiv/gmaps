...
- `map.show()` now rejects when the map element can't be found, instead of never settling. The selector not matching anything makes the internal setup throw, and that failure had nowhere to go — so a typo in the map selector left `show()` waiting forever and reported itself as an unhandled rejection somewhere else. The same failure reaching `init()` through the loader is handled now too.
