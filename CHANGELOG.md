...
- A tooltip or popup attached to a polyline no longer causes a hidden polyline to be drawn. The polyline dispatches its `ready` event so that the tooltip or popup can set up its events, and those events are added to the Google polyline when the polyline is shown. This means that `ready` can now be dispatched for a hidden polyline before the Google polyline exists. Use `toGoogle()` if you need the Google polyline, which creates it if it doesn't exist yet.
