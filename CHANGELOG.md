...
- Added the `Button` control and the `button()` function, in their own entry point at `@aptuitiv/gmaps/button`. Taking a button off a map and adding it back, or moving it from one map to another, keeps it working. An element that isn't a native button gets `role="button"`, `tabindex="0"` and Enter/Space activation, because attaching a click listener to a `div` is what makes it interactive in the first place; a real button, a link with an `href`, or an element with a `role` or `tabindex` already set is left as it is. It handles clicks, and optionally an enabled state and an active state, with the implementor saying what each state looks like through a mapping of attributes, class names, text and tooltips. A disabled button ignores clicks, and disabling also clears the active state. It's on `G` in the standalone browser build.
- Added the `LocationControl` control and the `locationControl()` function, in their own entry point at `@aptuitiv/gmaps/location-control`. Removing it stops it listening to the map and stops the watch it started, whether or not it had been added to the map yet, and moving it to another map lets go of the one it came from. Marker `svgIcon` options are merged into the default icon rather than replacing it, so overriding just the colour keeps the shape. It shows the user's position as a marker that follows them and gives them a control that takes the map back to it, which is the half of `map.locate()` the library didn't have. By default the control only appears once a location has been found, so a denied permission leaves nothing behind; `showWhenLocated: false` puts it on the map straight away instead. Later fixes move the marker without moving the map. The marker defaults to the usual blue dot and can be replaced, changed or left out. It's on `G` in the standalone browser build.
...
- `require('@aptuitiv/gmaps/button')` and `require('@aptuitiv/gmaps/location-control')` now return the control they name. Every subpath's `require` condition resolves to the one CommonJS build, and that build was made from the main entry point, which deliberately leaves those two out — so the modules loaded but the exports were undefined. The CommonJS build now includes them.
- `map.show()` now loads the Google Maps library itself when it isn't loaded yet, rather than waiting for a "load" event that is only dispatched on success. Waiting meant that a failed load, or nothing ever calling `load()`, left `show()` unsettled forever. It shares the load already in flight when there is one, so this doesn't start a second.
- Markers, polylines, overlays, geocoding, bounds, search boxes and marker clusters no longer wait forever when the Google Maps library can't be loaded. They all waited on the loader's `map_load` event, which is only dispatched on success, so a failed load left them waiting with nothing drawn, no error, and nothing to say why. They now reject — or log, where nothing is waiting on a promise. Thirteen more places that swallowed a rejection from those objects were fixed at the same time, so the failure reaches whoever asked rather than stopping halfway.
- Markers, polylines, overlays and data layers no longer leave an unhandled rejection behind when they start a map loading and that load fails. They kick off `map.init()` and don't wait on it, so now that it rejects the failure is logged instead of surfacing as an unhandled error pointing at the library.
- `map.show()` now rejects when the map element can't be found, instead of never settling, and says so again on a second attempt rather than hanging. The internal "setting up" flag was raised before the element was checked, so a failure left it raised and the next attempt waited on a ready event that was never coming. The selector not matching anything makes the internal setup throw, and that failure had nowhere to go — so a typo in the map selector left `show()` waiting forever and reported itself as an unhandled rejection somewhere else. The same failure reaching `init()` through the loader is handled now too.
- `map.init()` now rejects when the Google Maps library can't be loaded, and settles every caller waiting on it. It had no reject at all, so a failure — a missing API key, a network problem — escaped as an unhandled rejection and left the promise unsettled, with anything waiting on the map waiting for one that was never coming. Callers that arrived while an initialization was already running were worse off again: they waited on the "ready" event, which only fires on success. They now share the initialization that is in flight and are rejected with it, and a failed attempt no longer blocks a later one.
- `loader().load()` now settles every caller when a load fails. Callers arriving while a load was already running waited on the "load" event, which only fires on success, so a failed load left all but the first of them waiting for something that was never coming. They now share the load that is in flight and are rejected with it. The failed load is also let go of, so a later call starts a fresh attempt rather than being handed the failure again — before, nothing could make the loader try again at all.
