...
- Added the `LocationControl` control and the `locationControl()` function, in their own entry point at `@aptuitiv/gmaps/location-control`. Removing it stops it listening to the map and stops the watch it started, whether or not it had been added to the map yet, and moving it to another map lets go of the one it came from. It shows the user's position as a marker that follows them and gives them a control that takes the map back to it, which is the half of `map.locate()` the library didn't have. The control only appears once a location has been found, so a denied permission leaves nothing behind, and later fixes move the marker without moving the map. The marker defaults to the usual blue dot and can be replaced, changed or left out. It's on `G` in the standalone browser build.
